...
var CLAP_EXT_THREAD_POOL*: UncheckedArray[char] = "clap.thread-pool"

type
  ClapPluginThreadPoolT* {.bycopy.} = object
    exec*: proc (plugin: ptr ClapPluginT; taskIndex: uint32T) ##  Called by the thread pool

  ClapHostThreadPoolT* {.bycopy.} = object
    requestExec*: proc (host: ptr ClapHostT; numTasks: uint32T): bool ##  Schedule num_tasks jobs in the host thread pool.
