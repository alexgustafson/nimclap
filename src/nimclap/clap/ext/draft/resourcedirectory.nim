...
##     `PluginResourceDirectory.getFilesCount`
##     `PluginResourceDirectory.getFilePath`
...
type
  PluginResourceDirectory* {.bycopy.} = object
    ...
    getFilePath*: proc(
      ...
      ## `pathSize`: number of available bytes in path
  ...
  HostResourceDirectory* {.bycopy.} = object
    ...
    releaseDirectory*: proc(host: ptr Host, isShared: bool) {.cdecl.}
      ...
      ## If `isShared` = false, then the host may delete the directory content.
      ## `[main-thread]`
