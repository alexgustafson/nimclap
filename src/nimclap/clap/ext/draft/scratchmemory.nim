...
let CLAP_EXT_SCRATCH_MEMORY*: cstring = cstring"clap.scratch-memory/1"

type
  clap_host_scratch_memory* {.bycopy.} = object
    ...
    reserve*: proc (host: ptr clap_host; scratch_size_bytes: uint32;
                  max_concurrency_hint: uint32): bool {.cdecl.}
    ...
    ##
    ##  [audio-thread]
    access*: proc (host: ptr clap_host): pointer {.cdecl.}
