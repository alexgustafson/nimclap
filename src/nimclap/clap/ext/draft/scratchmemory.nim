...
## `Plugin.process` and `PluginThreadPool.exec`;
...
let extScratchMemory*: cstring = cstring"clap.scratch-memory/1"

type HostScratchMemory* {.bycopy.} = object
  reserve*: proc(
    host: ptr Host, scratchSizeBytes: uint32, maxConcurrencyHint: uint32
  ): bool {.cdecl.}
    ...
    ##
    ## `maxConcurrencyHint` is an optional hint which indicates
  ...
  access*: proc(host: ptr Host): pointer {.cdecl.}
    ## Returns a pointer to the "thread-local" scratch memory.
    ##
    ## If the scratch memory wasn't successfully reserved, returns nil.
    ##
    ## If the plugin crosses `maxConcurrencyHint`, then the return value
    ## is either nil or a valid scratch memory pointer.
    ##
    ## This method may only be called by the plugin from the audio thread,
    ## (i.e. during the `process` or `PluginThreadPool.exec` callback), and
    ...
    ## `HostThreadPool.requestExec` and clear the cached pointer
