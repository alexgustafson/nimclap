...
const
  ...
  paramIsStepped* = 1 shl 0 ##  Useful for periodic parameters like a phase
  ...
  paramIsHidden* = 1 shl 2 ##  The parameter can't be changed by the host.
  ...
  paramIsAutomatable* = 1 shl 5 ##  Does this parameter support per note automations?
  ...
  paramIsModulatable* = 1 shl 10 ##  Does this parameter support per note modulations?
...
type
  ParamInfo* {.bycopy.} = object ##  Stable parameter identifier, it must never change.
    id*: Id
    ...
    defaultValue*: cdouble ##  Default plain value. Must be in [min, max] range.
...
const ##  Clears all possible references to a parameter
  paramClearAll* = 1 shl 0 ##  Clears all automations to a parameter
  paramClearAutomations* = 1 shl 1 ##  Clears all modulations to a parameter
